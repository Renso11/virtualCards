...
class CarteVirtuelleController extends Controller
{
    ...
    public function venteVirtuellesAttentes(Request $request)
    { 
        try {
            $ventes = VenteVirtuelle::where('deleted',0)->where('etat',null)->orderBy('id','desc')->get();
            $countries = $this->countries;
            return view('ventesVirtuelles.index',compact('ventes','countries'));
        } catch (\Exception $e) {
            return back()->withError($e->getMessage());
        } 
    }
    public function venteVirtuellesDelete(Request $request)
    { 
        try {
            $vente = VenteVirtuelle::where('id',$request->id)->first();
            $vente->deleted = 1;
            $vente->save();
    ...
    public function venteVirtuellesAttentesValidation(Request $request)
    { 
        try {
            ...
            // Enregistrer la carte virtuelle

            $carteVendu = CarteVirtuelle::create([
            ...
            Mail::to([$vente->kycClient->email,])->send(new MailVenteVirtuelle(['registrationAccountId' => $responseBody->registrationAccountId,'registrationLast4Digits' => $responseBody->registrationLast4Digits,'registrationPassCode' => $responseBody->registrationPassCode]));
            
            $vente->etat = 1;
            $vente->validateur_id = 1;
            $vente->carte_virtuelle_id = $carteVendu->id;
    ...
    public function venteVirtuellesAttentesRejet(Request $request)
    { 
        try {
            $vente = VenteVirtuelle::where('id',$request->id)->first();
            $vente->etat = 0;
    ...
    public function venteVirtuellesFinalises(Request $request)
    { 
        try {
            $ventes = VenteVirtuelle::where('deleted',0)->where('etat',1)->orderBy('id','desc')->get();
            return view('ventesVirtuelles.finalises',compact('ventes'));
        } catch (\Exception $e) {
            return back()->withError($e->getMessage());
        } 
    }

    public function venteVirtuellesRejetes(Request $request)
    { 
        try {
            $ventes = VenteVirtuelle::where('deleted',0)->where('etat',0)->orderBy('id','desc')->get();
            return view('ventesVirtuelles.rejetes',compact('ventes'));
        } catch (\Exception $e) {
            return back()->withError($e->getMessage());
        } 
    }
